...
int main(int argc, char *argv[]) {
    ...
    if (pid == 0) {
        ...
        srand(time(NULL));
        number_to_guess = rand() % N + 1;  // Генерация единственного числа
        ...
        while (1) {
            ...
            if (guessed) {
                printf("Process %d: Number %d guessed correctly!\n", getpid(), number_to_guess);
                printf("Process %d: Attempts: %d\n", getpid(), attempts);
                number_to_guess = rand() % N + 1;  // Генерация нового числа
